 ...
 */
template <typename T>
constexpr __host__ __device__ inline cuda::std::enable_if_t<cuda::std::is_integral_v<T>, bool> is_pow2(T v)
{
  return (v && !(v & (v - 1)));
}
...
struct Pow2 {
  ...
  static_assert(cuda::std::is_integral<Type>::value, "Value must be integral.");
...
#define Pow2_WHEN_INTEGRAL(I)       cuda::std::enable_if_t<Pow2_IS_REPRESENTABLE_AS(I), I>
#define Pow2_IS_REPRESENTABLE_AS(I) (cuda::std::is_integral<I>::value && Type(I(Value)) == Value)
  ...
  {
    if constexpr (cuda::std::is_signed<I>::value) return (x >> I(Log2)) + (x < 0 && (x & I(Mask)));
    if constexpr (cuda::std::is_unsigned<I>::value) return x >> I(Log2);
  }
  ...
  {
    if constexpr (cuda::std::is_signed<I>::value) return x < 0 ? -((-x) & I(Mask)) : (x & I(Mask));
    if constexpr (cuda::std::is_unsigned<I>::value) return x & I(Mask);
  }
...
#define Pow2_CHECK_TYPE(T)                                               \
  static_assert(cuda::std::is_pointer<T>::value || cuda::std::is_integral<T>::value, \
