...
struct loadAndComputeDist {
  ...
  __device__ __forceinline__ loadAndComputeDist(AccT& dist, Lambda op, AccT& norm_query, AccT& norm_data)
...
struct loadAndComputeDist<kUnroll, Lambda, uint8_veclen, uint8_t, uint32_t, ComputeNorm> {
  ...
  __device__ __forceinline__ loadAndComputeDist(uint32_t& dist, Lambda op, uint32_t& norm_query, uint32_t& norm_data)
    : dist(dist), compute_dist(op), norm_query(norm_query), norm_data(norm_data)
  {
  }
...
struct loadAndComputeDist<kUnroll, Lambda, int8_veclen, int8_t, int32_t, ComputeNorm> {
  ...
  __device__ __forceinline__ loadAndComputeDist(int32_t& dist, Lambda op, int32_t& norm_query, int32_t& norm_data)
    : dist(dist), compute_dist(op), norm_query(norm_query), norm_data(norm_data)
  {
  }
...
#pragma unroll
    for (int j = 0; j < kUnroll; ++j) {
      ...
      if constexpr (ComputeNorm) {
        ...
        norm_data  += int32_t{data[loadIndex + j * kIndexGroupSize]} * int32_t{data[loadIndex + j * kIndexGroupSize]};
      }
    }
  }
    ...
    for (int probe_id = blockIdx.x; probe_id < n_probes; probe_id += gridDim.x) {
      ...
      for (uint32_t group_id = align_warp::div(threadIdx.x); group_id < num_groups;
        ...
        if (valid) {
          ...
          loadAndComputeDist<kUnroll, decltype(compute_dist), Veclen, T, AccT, ComputeNorm> lc(dist,
                                                                                  compute_dist, norm_query, norm_dataset);
          ...
          if (dim > query_smem_elems) {
            ...
            loadAndComputeDist<1, decltype(compute_dist), Veclen, T, AccT, ComputeNorm> lc(dist, compute_dist, norm_query, norm_dataset);
        ...
        if constexpr (ComputeNorm) {
          if (valid)
            val = val / (raft::sqrt(static_cast<float>(norm_query)) * raft::sqrt(static_cast<float>(norm_dataset)));
        }
        if constexpr (kManageLocalTopK) {
