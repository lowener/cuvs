...
template <typename T1,
          ...
          cuda::std::enable_if_t<
            CUDA_CONDITION_ELSE_TRUE(RAFT_DEPAREN((
...
template <typename T>
RAFT_DEVICE_INLINE_FUNCTION typename cuda::std::enable_if_t<cuda::std::is_same_v<T, __half>, __half>
min(T x, T y)
{
...
RAFT_DEVICE_INLINE_FUNCTION
  typename cuda::std::enable_if_t<cuda::std::is_same_v<T, nv_bfloat16>, nv_bfloat16>
  min(T x, T y)
{
...
{
  return cuvs::tools::max(x, cuvs::tools::max(y, cuda::std::forward<Args>(args)...));
}
