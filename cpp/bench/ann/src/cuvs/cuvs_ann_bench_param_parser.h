...
{
  if (conf.is_null() || !conf.value("dynamic_batching", false)) { return; }
                       ...
                       typename cuvs::bench::cuvs_cagra<T, IdxT>::build_param& param)
{
  ...
  if (conf.contains("nn_descent_bbq_query_format") != conf.contains("nn_descent_bbq_doc_format")) {
    throw std::runtime_error(
      "nn_descent_bbq_query_format and nn_descent_bbq_doc_format must be given together");
  }
  if (conf.contains("nn_descent_bbq_query_format")) {
    using bbq_param = typename cuvs::bench::cuvs_cagra<T, IdxT>::build_param::bbq_param;
    param.bbq =
      bbq_param{conf.at("nn_descent_bbq_query_format"), conf.at("nn_descent_bbq_doc_format")};
  }
