              ...
              raft::host_matrix_view<const int64_t, int64_t, raft::row_major> neighbors_gt)
{
  ...
  auto transformed_dataset   = raft::make_host_matrix<uint8_t, int64_t>(n_vectors, pq_dim);
  ...
  pq.train(n_vectors, dataset_cpu.data_handle());
                ...
                raft::host_matrix_view<const int64_t, int64_t, raft::row_major> neighbors_gt)
{
  ...
  auto out =
    raft::make_device_matrix<uint8_t, int64_t, raft::row_major>(res, dataset.extent(0), pq_dim);
  auto neighbors =
    raft::make_device_matrix<indexing_dtype, indexing_dtype>(res, n_queries, k_neighbors);
            ...
            bool kmeans_balanced)
{
  ...
  raft::resource::sync_stream(res);
  auto start     = std::chrono::high_resolution_clock::now();
  auto quantizer = quantize::pq::train(res, params, raft::make_const_mdspan(dataset));
  raft::resource::sync_stream(res);
  auto end           = std::chrono::high_resolution_clock::now();
  auto duration      = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  double training_ms = duration.count();
  ...
  quantize::pq::transform(
    res, quantizer, raft::make_const_mdspan(dataset), transformed_dataset.view());
  raft::resource::sync_stream(res);
  end                  = std::chrono::high_resolution_clock::now();
  duration             = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
  results.transform_ms = duration.count();
...
int main(int argc, char** argv)
{
  ...
  if (args.method != "complete") {
    ...
    for (uint32_t pq_bits : {4u, 6u, 7u, 8u}) {
    ...
    write_csv("pq_timing_results.csv", all_results);
  }
